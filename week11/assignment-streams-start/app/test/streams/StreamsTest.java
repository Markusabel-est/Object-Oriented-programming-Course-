...
public class StreamsTest {
    ...
    public void countEvenNumbers() {
        ...
        long result = Stream.of(PI).filter(x -> x % 2 == 0).count();
    ...
    public void sumOddNumbers() {
        ...
        int result =Stream.of(PI).filter(x -> x % 2 != 0).mapToInt(x -> x).sum() ;
    ...
    public void multiplyNumbers() {
        ...
        int result = Stream.of(PI).sorted((a,b) -> b-a).limit(5).mapToInt(x -> x).reduce(1, (a,b) -> a*b);
    ...
    public void calculateTax() {
        ...
        Double result = Stream.of(prices).map(x -> x *(1 + tax)).reduce(0.0, (a,b) -> a+b);
    ...
    public void sumStringIntegers() {
        ...
        int result = Stream.of(input).mapToInt(Integer::parseInt).sum();
    ...
    public void streamOfStreams() {
        ...
        int result =Stream.of(input).mapToInt(row -> Arrays.stream(row).reduce(0, Integer::sum)).reduce(0, Integer::sum);    // Stream<int[]>
    ...
    public void everySecondElement() {
        ...
        List<Integer> result = IntStream.range(0, input.size()).filter(i -> i % 2 == 0).mapToObj(input::get).collect(Collectors.toList());
    ...
    public static String rot13(String s) {
        ...
        s =s.chars().map(x -> rot13(x)).mapToObj(x -> String.valueOf((char)x)).collect(StringBuilder::new, StringBuilder::append, StringBuilder::append).toString();
        return s;
    }
    ...
    public void philosophers() {
        ...
        Map<String,String> result = philosophers.entrySet().stream().filter(e -> e.getValue().equals("Greek")).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
